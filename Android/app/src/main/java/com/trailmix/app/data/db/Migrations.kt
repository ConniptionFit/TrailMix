...
object Migrations {
    ...
    // REL-18: two indices for columns every hot query filters on — additive, no data touched.
    // Names must match Room's own auto-generated convention (index_<table>_<column>) exactly,
    // or schema validation fails at runtime with a mismatch against what Room expects to exist.
    val MIGRATION_13_14 = object : Migration(13, 14) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL("CREATE INDEX IF NOT EXISTS index_notes_deletedAtEpochMs ON notes(deletedAtEpochMs)")
